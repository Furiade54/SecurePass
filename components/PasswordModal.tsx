...
const PasswordModal: React.FC<PasswordModalProps> = ({ isOpen, onClose, onSave, onOpenGenerator, generatedPassword, existingEntry, categories }) => {
  ...
  const filteredCategories = categories.filter(c => c.toLowerCase().includes(category.toLowerCase()));

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-40 p-4" onClick={() => setShowSuggestions(false)}>
      <div className="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md text-gray-200 animate-fade-in-up" onClick={e => e.stopPropagation()}>
